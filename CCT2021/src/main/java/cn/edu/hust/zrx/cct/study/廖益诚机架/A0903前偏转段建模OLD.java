...
import static cn.edu.hust.zrx.cct.base.BaseUtils.Constant.MRAD;
...
public class A0903前偏转段建模OLD {

    enum COSY_MAP {
        part1Optics, part1beforeFirstQS1;
    ...
    @Run(1)
    public void 参数打印() {
        Logger.getLogger().info("agCctAngle1 = " + agCctAngle1);
        Logger.getLogger().info("agCctAngle2 = " + agCctAngle2);
    }

    @Run(2)
    public void 前偏转段CCT12建模() {
        ...
        List<List<Point2>> lists = cct12_1.multiplePoleMagnetAlongTrajectoryBreak(
                trajectoryPart1, MM, 10 * MM, 2, 6
        );
        ...
        Plot2d.plot2(b, Plot2d.BLUE_LINE);

        Plot2d.info("s/m", "B", "", 18);

        Plot2d.showThread();
    }

    @Run(value = 3, validate = false) // 调整到 6191.87824
    public void 调整前段二极CCT电流() {
        agCct12IInner = 0.0;
        agCct12IOuter = 0.0;

        final double dipoleCct12IInner0 = dipoleCct12IInner;

        final Trajectory trajectoryPart1 = getTrajectoryPart1();

//        dipoleCct12IInner = 5879.4; // 求解获得
//        dipoleCct12IOuter = dipoleCct12IInner;


        List<Point2> point2List = BaseUtils.Python.linspaceStream(6191.877955555556, 6191.878488888889, 16)
                .sequential()
                .mapToObj(k -> {
                    dipoleCct12IInner = k;
                    dipoleCct12IOuter = dipoleCct12IInner;
                    ...
                    Cct cct12_2 = getCct12_2();

                    Elements elements = Elements.of(cct12_1, cct12_2);

                    return BaseUtils.Content.BiContent.create(dipoleCct12IInner, elements);
                ...
                .map(bi -> {
                    ...
                    ParticleRunner.run(ip, elements, trajectoryPart1.getLength(), MM);

                    Vector2 v = ip.getVelocity().toVector2();

                    Vector2 directAtEnd = trajectoryPart1.directAtEnd();
    ...
    public void 四极场调整() {
        ...
        List<List<Point2>> lists = elements.multiplePoleMagnetAlongTrajectoryBreak(
                trajectoryPart1, MM, 10 * MM, 2, 6
        );
        ...
        Plot2d.plot2(g, Plot2d.BLUE_LINE);

        Plot2d.info("s/m", "B/m", "", 18);
    ...
    public void 前段磁场分布() {
        ...
        List<List<Point2>> lists = elementsOfAllPart1.multiplePoleMagnetAlongTrajectoryBreak(
                trajectoryPart1, MM, 10 * MM, 2, 6
        );
        ...
        Plot2d.plot2(l, Plot2d.BLUE_LINE);

        Plot2d.info("s/m", "T", "二极场分布", 18);
//        Plot2d.info("s/m", "T/m", "四极场分布", 18);
        Plot2d.info("s/m", "T/m2", "六极场分布", 18);


        Plot2d.showThread();
    }

    @Run(value = 100, validate = true) // 完了
    public void 前段粒子跟踪() {
...
//        Plot3d.showThread();
    ...
    @Run(value = 7, validate = false)
    public void 研究下色散() {
        MagnetAble elementsOfAllPart1 = getElementsOfAllPart1();
        ...
        List<Point2> list = track色散(trajectoryPart1, elementsOfAllPart1);

        Plot2d.plot2(list, Plot2d.BLUE_LINE);

        Plot2d.info("s/m", "mm/%", "色散", 18);
    ...
    public void 单粒子跟踪() {
        ...
        Line2 trajectoryPart1 = getTrajectoryPart1();//.resetLength(DL1 + CCT12_LENGTH + GAP1);
    ...
    public void 消除二极CCT四极场() {
        ...
        String[] des = BaseUtils.Python.linspaceStream(-0.5, 0.5, switcher.getSize())
                .sequential()
                .mapToObj(k -> {
                    ...
                    return BaseUtils.Content.BiContent.create(k, cct12_1);
                }).collect(Collectors.toList())
                .stream()
                .parallel()
                .map(bi -> {
...
//        boolean directθInner, boolean directθOuter
    ...
    private final int dipoleCct345WindingNumber = 0; // 原 180，让最小匝间距>5mm
    private final int agCctWindingNumber3 = 0;
    private final int agCctWindingNumber4 = 0;
    private final int agCctWindingNumber5 = 0;  // sum 135
    ...
    private final double agCctAngle5 = 0.0;
    ...
    private double dipoleCct345IInner = -10 * kA;
    private double dipoleCct345IOuter = dipoleCct345IInner;
    private double agCct345IInner = 4000; // 9799 // 2020年6月11日 *0.995
