...
/**
 * Description
 * C20201026廖益诚10月机架建模双极坐标系版本_修改绕线和smallR极点
 * <p>
 * Data
 * 9:12
 *
...
public class C20201026廖益诚10月机架建模双极坐标系版本_修改绕线和smallR极点 {

    @Run(1)
    public void 新形势下二极CCT建模() {
        // 研究一下
        // R^2 = a^2 + r^2

        GantryDataBipolarCo.FirstBend firstBend = firstBend();
        GantryDataBipolarCo.SecondBend secondBend = secondBend();
        Cct cct = dipoleCct345_local_goodWindingMethod_a_as_bigR(secondBend);
        Line2 line2 = testTrajectory675();
        List<Point2> bz = cct.magnetBzAlongTrajectoryParallel(line2);
        List<Point2> g = cct.magnetGradientAlongTrajectoryFast(line2, 10 * MM, 10 * MM);
...
//        Plot2d.plot2(bz);
    ...
    public void 移动二极场至最终位置() {
        ...
        List<Point2> bz = moveCCT3451.magnetBzAlongTrajectoryParallel(trajectory2);
    ...
    @Run(5)
    public void 拭目以待的四极CCT() {
        Cct cct = agCct345_local_goodWindingMethod_a_as_bigR(secondBend());
...
//        Plot2d.plot2(g);
    ...
    public void 多粒子跟踪(){
        ...
        CctUtils.trackMultiParticles2d(
                32,0.0,trajectory2,trajectory2.getLength(),elements,false,215
        ).stream().map(point2s -> Point2.convert(point2s,1,1/MM)).forEach(point2s -> Plot2d.plot2(point2s,Plot2d.BLACK_LINE));
    ...
    public void 改变四极场下的相椭圆对比2() {
        ...
        BaseUtils.Switcher<String> switcher = CctUtils.createPlotDescribeSwitcher();
        List<String> des = BaseUtils.Python.linspaceStream(-6000, -7500, switcher.getSize() - 1)
                .sequential()
                .peek(i -> {
                    GantryDataBipolarCo.FirstBend firstBend = firstBend();
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
...
//        );
    ...
    private Elements allElementPart2(GantryDataBipolarCo.FirstBend firstBend, GantryDataBipolarCo.SecondBend secondBend) {
        Cct all = dipoleCct345_local_goodWindingMethod_a_as_bigR(secondBend)
                .addCct(agCct345_local_goodWindingMethod_a_as_bigR(secondBend));
    ...
    public Cct dipoleCct345_local_goodWindingMethod_a_as_bigR(GantryDataBipolarCo.SecondBend secondBend) {
        ...
        MathFunction phiKsiFun = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phi0, tiltAngles);
        SoleLayerCct soleLayerCct = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phiKsiFun, I0,
                0, endKsi, windingNumber * disperseNumberPerWinding);
        cct.addSoleLayerCct(soleLayerCct);


        MathFunction phiKsiFun1 = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + outerSmall * outerSmall), outerSmall, phi0, tiltAngles);
        phiKsiFun1 = phiKsiFun1.yAxisSymmetry();
        SoleLayerCct soleLayerCct1 = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + outerSmall * outerSmall), outerSmall, phiKsiFun1, I0,
                0, -endKsi, windingNumber * disperseNumberPerWinding);
    ...
    public Cct agCct345_local_goodWindingMethod_a_as_bigR(GantryDataBipolarCo.SecondBend secondBend) {
        ...
        MathFunction phiKsiFunInner1 = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phi0[0], tiltAngles);
        SoleLayerCct soleLayerCctInner1 = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phiKsiFunInner1, I0,
                0, endKsis[0], windingNumbers[0] * 360);
        cct.addSoleLayerCct(soleLayerCctInner1);

        MathFunction phiKsiFunInner2 = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phi0[1], BaseUtils.ArrayUtils.dot(tiltAngles, -1))
                .yAxisSymmetry().move(Vector2.create(endKsis[0], bendingRadians[0] + phi0[0]));
        SoleLayerCct soleLayerCctInner2 = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phiKsiFunInner2, I0,
                endKsis[0], endKsis[0] - endKsis[1], windingNumbers[1] * 360);
        cct.addSoleLayerCct(soleLayerCctInner2);

        MathFunction phiKsiFunInner3 = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phi0[2], tiltAngles)
                .move(Vector2.create(endKsis[0] - endKsis[1],
                        bendingRadians[0] + bendingRadians[1] + phi0[0] + phi0[1]));
        SoleLayerCct soleLayerCctInner3 = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + innerSmallR * innerSmallR), innerSmallR, phiKsiFunInner3, I0,
                endKsis[0] - endKsis[1], endKsis[0] - endKsis[1] + endKsis[2], windingNumbers[2] * 360);
        cct.addSoleLayerCct(soleLayerCctInner3);


        MathFunction phiKsiFunOuter1 = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + outerSmallR * outerSmallR), outerSmallR, phi0[0], tiltAngles)
                .yAxisSymmetry();
        SoleLayerCct soleLayerCctOuter1 = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + outerSmallR * outerSmallR), outerSmallR, phiKsiFunOuter1, I0,
                0, -endKsis[0], windingNumbers[0] * 360);
        cct.addSoleLayerCct(soleLayerCctOuter1);
        MathFunction phiKsiFunOuter2 = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + outerSmallR * outerSmallR), outerSmallR, phi0[1], BaseUtils.ArrayUtils.dot(tiltAngles, -1))
                .yAxisSymmetry().move(Vector2.create(endKsis[0], bendingRadians[0] + phi0[0]))
                .yAxisSymmetry();
        SoleLayerCct soleLayerCctOuter2 = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + outerSmallR * outerSmallR), outerSmallR, phiKsiFunOuter2, I0,
                -endKsis[0], -(endKsis[0] - endKsis[1]), windingNumbers[1] * 360);
        cct.addSoleLayerCct(soleLayerCctOuter2);
        MathFunction phiKsiFunOuter3 = MathFunction.createPhiKsiFun(Math.sqrt(bigR * bigR + outerSmallR * outerSmallR), outerSmallR, phi0[2], tiltAngles)
                .move(Vector2.create(endKsis[0] - endKsis[1],
                        bendingRadians[0] + bendingRadians[1] + phi0[0] + phi0[1]))
                .yAxisSymmetry();
        SoleLayerCct soleLayerCctOuter3 = SoleLayerCct.createSoleLayerCct(Math.sqrt(bigR * bigR + outerSmallR * outerSmallR), outerSmallR, phiKsiFunOuter3, I0,
                -(endKsis[0] - endKsis[1]), -(endKsis[0] - endKsis[1] + endKsis[2]), windingNumbers[2] * 360);
