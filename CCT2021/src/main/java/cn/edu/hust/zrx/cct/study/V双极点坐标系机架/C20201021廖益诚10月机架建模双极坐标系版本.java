...
public class C20201021廖益诚10月机架建模双极坐标系版本 {

    @Run(1)
    @Deprecated
    public void 二极CCT电流调整0() {
        GantryDataBipolarCo.FirstBend firstBend = firstBend();
        GantryDataBipolarCo.SecondBend secondBend = secondBend();
        Cct dipoleCct345 = dipoleCct345_local(secondBend);
        dipoleCct345 = moveCCT345_1(dipoleCct345, firstBend, secondBend);

        Trajectory trajectory2 = GantryDataBipolarCoUtils.getTrajectory2(firstBend, secondBend);


//        List<Point2> bz = dipoleCct345.magnetBzAlongTrajectory(trajectory2);
//        List<Point2> bz1 = dipoleCct345.magnetBzAlongTrajectoryParallel(trajectory2);
//
//        Plot2d.plot2(bz);
//        Plot2d.plot2(bz1);

        final double distance = secondBend.DL2 + secondBend.CCT345_LENGTH + secondBend.GAP3 * 2 + secondBend.QS3_LEN;

        List<Point2> y = CctUtils.trackingIdealParticle(trajectory2, distance, dipoleCct345, false, 215);
        List<Point2> x = CctUtils.trackingIdealParticle(trajectory2, distance, dipoleCct345, true, 215);

        Plot2d.plot2(x);
//        Plot2d.plot2(y); // y  方向无偏
    ...
    @Run(2)
    @Deprecated
    public void 二极CCT电流调整1() {
        List<Point2> ir = BaseUtils.Python.linspaceStream(-9443.838064516129, -9443.830322580645, 32)
                .parallel()
                .mapToObj(i -> {
                    GantryDataBipolarCo.FirstBend firstBend = firstBend();
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
                    ...
                    ParticleRunner.run(ip, dipoleCct345, distance, MM);

                    Vector2 v2 = ip.velocity.toVector2();
                    Vector2 d2 = trajectory2.directAt(distance);
                    double r = v2.angleToInRadian(d2);

                    return Point2.create(i, r);
                })
    ...
    @Run(3)
    public void 四极CCT磁场分布() {
        GantryDataBipolarCo.SecondBend secondBend = secondBend();
        GantryDataBipolarCo.FirstBend firstBend = firstBend();
        Cct agCct345_local = agCct345_local(secondBend);
        Cct agcct345_1 = moveCCT345_1(agCct345_local, firstBend, secondBend);

        Trajectory trajectory2 = GantryDataBipolarCoUtils.getTrajectory2(firstBend, secondBend);

        List<Point2> g = agcct345_1.magnetGradientAlongTrajectoryFast(trajectory2, MM, 10 * MM);
    ...
    public void 全段() {
        ...
        List<List<Point2>> lists = elements.multiplePoleMagnetAlongTrajectoryBreak(trajectory2, MM, 10 * MM, 2, 12);
    ...
    @Run(5)
    public void 相椭圆我来了() {
        Trajectory trajectory2 = trajectory2Default();
        Elements ele = allElementPart2Default();
    ...
    @Run(6)
    public void cosy相椭圆() {
        List<Point2> point2s = CctUtils.cosyPhaseEllipse(
                false, 0, 512, 5, false, 1, COSY_MAP1021.part2.map
        );

        Plot2d.plot2(point2s);
    ...
    public void 多粒子跟踪() {
        GantryDataBipolarCo.FirstBend firstBend = firstBend();
        GantryDataBipolarCo.SecondBend secondBend = secondBend();
        secondBend.agCct345I0 = -7550;
        secondBend.dipoleCct345I0 = -10411;
        ...
        List<List<Point2>> lists = CctUtils.trackMultiParticles2d(
                32, 0, t, t.getLength(), e, false, 215);

        lists.forEach(ps -> Plot2d.plot2(ps, Plot2d.BLACK_LINE));
    ...
    public void 修改四极CCT电流() {
        ...
        List<String> des = BaseUtils.Python.linspaceStream(-7600, -7500, 6)
                .parallel()
                .mapToObj(i -> {
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
                    ...
                    return BaseUtils.Content.BiContent.create(i, point2s);
                })
                .collect(Collectors.toList())
                .stream()
                .sequential()
                .sorted(Comparator.comparingDouble(BaseUtils.Content.BiContent::getT1))
                .peek(bi -> Plot2d.plot2circle(bi.getT2(), switcher.getAndSwitch()))
                .map(BaseUtils.Content.BiContent::getT1)
                .map(i -> "I=" + i)
                .collect(Collectors.toList());

        Plot2d.info("x/mm", "xp/mr", "", 18);
//        Plot2d.info("y/mm","yp/mr","",18);
    ...
    @Run(-20)
    public void 二极CCT电流调整2() {
        List<Point2> ir = BaseUtils.Python.linspaceStream(-10451, -10387, 32)
                .parallel()
                .mapToObj(i -> {
                    GantryDataBipolarCo.FirstBend firstBend = firstBend();
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
                    ...
                    secondBend.dipoleCct345I0 = i;
                    secondBend.agCct345I0 = -7750;

                    Elements e = allElementPart2(firstBend, secondBend);

                    Trajectory trajectory2 = GantryDataBipolarCoUtils.getTrajectory2(firstBend, secondBend);

                    final double distance = trajectory2.getLength();

                    RunningParticle ip = ParticleFactory.createIdealProtonAtTrajectory(trajectory2, 215);
                    ParticleRunner.run(ip, e, distance, MM);
    ...
    @Run(-100)
    public void 画图() {
        Cct cct = dipoleCct345_local(secondBend());
        cct.plot3(Plot2d.BLUE_LINE, Plot2d.BLUE_LINE);

        List<Point3> w1 = cct.getSoleLayerCctList().get(0).windings;
        Point3 p0 = w1.get(w1.size() - 1);
        List<Point3> w2 = cct.getSoleLayerCctList().get(1).windings;
        Point3 p1 = w2.get(w2.size() - 1);

        Plot3d.plotPoint(p0, Plot2d.BLACK_SMALL_POINT);
        Plot3d.plotPoint(p1, Plot2d.BLACK_SMALL_POINT);

        Plot3d.setCenter(Point3.create(0, 0.95, 0), 2);
    ...
    //-------------------------------  情况很不好，需要慢慢调整  ----------------------------------
    @Run(101) // 2020年10月22日 好家伙，有2T/m
    public void 消除二极CCT的四极场之四极场分布() {

        GantryDataBipolarCo.SecondBend secondBend = secondBend();

        secondBend.dipoleCct345TiltAngles[1] = 25;

        Cct diCct = dipoleCct345_local(secondBend);
...
//                .addStraitLine(1);
        ...
        Line2 trajectory = GantryDataBipolarCoUtils.getTrajectory2(firstBend(), secondBend)
                .resetLength(secondBend.DL2 + secondBend.CCT345_LENGTH + secondBend.GAP3 * 2 + secondBend.QS3_LEN);

        diCct = moveCCT345_1(diCct, firstBend(), secondBend);

        List<Point2> bz = diCct.magnetBzAlongTrajectory(trajectory);

        List<Point2> g = diCct.magnetGradientAlongTrajectoryFast(trajectory, MM, 10 * MM);
    ...
    @Run(-102) // 25 度
    public void 正式消除二极CCT的四极场() {
        BaseUtils.Switcher<String> s = CctUtils.createPlotDescribeSwitcher();
        List<String> des = BaseUtils.Python.linspaceStream(20, 40, s.getSize())
                .parallel()
                .mapToObj(tilt -> {
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
                    ...
                    List<Point2> g = diCct.magnetGradientAlongTrajectoryFast(trajectory, MM, 10 * MM);
//
//                    List<Point2> g = diCct.magnetBzAlongTrajectory(trajectory);
                ...
                .map(BaseUtils.Content.BiContent::getT1)
                .map(tilt -> "tilt_ang = " + tilt)
                .collect(Collectors.toList());

        Plot2d.info("s/m", "G/Tm-1", "", 18);
//        Plot2d.info("s/m","B/T","",18);
    ...
    @Run(103) // -9664
    public void 再次调整二极CCT二极场() {
        BaseUtils.Switcher<String> s = CctUtils.createPlotDescribeSwitcher();
        List<Point2> ir = BaseUtils.Python.linspaceStream(-9687.5, -9625.0, s.getSize())
                .parallel()
                .mapToObj(i -> {
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
                    ...
                    Line2 trajectory = GantryDataBipolarCoUtils.getTrajectory2(firstBend(), secondBend)
                            .resetLength(secondBend.DL2 + secondBend.CCT345_LENGTH + secondBend.GAP3);

                    RunningParticle ip = ParticleFactory.createIdealProtonAtTrajectory(trajectory, 215);

                    ParticleRunner.run(ip, diCct, secondBend.DL2 * 2 + secondBend.CCT345_LENGTH, MM);

                    Vector2 v2 = ip.velocity.toVector2();
                    Vector2 d2 = trajectory.directAtEnd();
    ...
    @Run(104)
    public void 消除四极CCT二极场之查看分布() {
        GantryDataBipolarCo.SecondBend secondBend = secondBend();

        Cct cct = agCct345_local(secondBend);

        Cct agcct = moveCCT345_1(cct, firstBend(), secondBend);

        Trajectory trajectory = GantryDataBipolarCoUtils.getTrajectory2(firstBend(), secondBend);

        List<Point2> bz = agcct.magnetBzAlongTrajectoryParallel(trajectory);
//
//        List<Point2> g = agcct.magnetGradientAlongTrajectoryFast(trajectory, MM, 10 * MM);
//
        Plot2d.plot2(bz);
//
//        Plot2d.plot2(g);
    ...
    @Run(105) // 无法消除 无法消除 无法消除
    public void 正式消除四极CCT二极场() {
        BaseUtils.Switcher<String> switcher = CctUtils.createPlotDescribeSwitcher();
        List<String> des = BaseUtils.Python.linspaceStream(-20, -50, switcher.getSize())
                ...
                .mapToObj(t -> {
                    ...
                    secondBend.agCct345TiltAngles[0] = t;
    ...
    @Run(-106)
    public void plotPhiKsiFunShow() {
        plotPhiKsiFun(secondBend(), Plot2d.RED_LINE, Plot2d.TRANSPARENCY_LINE);
    }

    @Run(107)
    public void 查看CCT建造成果() {
        GantryDataBipolarCo.FirstBend firstBend = firstBend();
        GantryDataBipolarCo.SecondBend secondBend = secondBend();

        Cct d = dipoleCct345_local(secondBend);
        Cct a = agCct345_local(secondBend);
        Cct cct = moveCCT345_1(d.addCct(a), firstBend, secondBend);

        Trajectory trajectory2 = GantryDataBipolarCoUtils.getTrajectory2(firstBend, secondBend);

        Trajectory trajectory = TrajectoryFactory.setStartingPoint(trajectory2.pointAtStart())
                .setStraightLine(secondBend.DL2, trajectory2.directAtStart())
                .addArcLine(0.95, true, 67.5)
                .addStraitLine(secondBend.DL2);

//        List<Point2> bz = cct.magnetBzAlongTrajectoryParallel(trajectory);
//
//        List<Point2> g = cct.magnetGradientAlongTrajectoryFast(trajectory, MM, 10 * MM);
//
//        Plot2d.plot2(bz);
//
//        Plot2d.plot2(g);
        ...
        List<Point2> cps = CctUtils.cosyPhaseEllipse(
                xPlane, 0, 512, 5, false, 1, COSY_MAP1021.cct345WithDl2TwoSides.map
        );

        Plot2d.plot2circle(cps, Plot2d.BLUE_LINE);
...
//        Plot2d.info("x/mm", "xp/mr", "", 18);
    ...
    public void 改变四极场下的相椭圆对比() {
        ...
        BaseUtils.Switcher<String> switcher = CctUtils.createPlotDescribeSwitcher();
        List<String> des = BaseUtils.Python.linspaceStream(-6000, -8000, switcher.getSize() - 1)
                .sequential()
                .peek(i -> {
                    GantryDataBipolarCo.FirstBend firstBend = firstBend();
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
    ...
    public void 全段四极CCT电流调节() {
        ...
        BaseUtils.Switcher<String> switcher = CctUtils.createPlotDescribeSwitcher();
        List<String> des = BaseUtils.Python.linspaceStream(-7142, -6857, switcher.getSize() - 1)
                .sequential()
                .peek(i -> {
                    GantryDataBipolarCo.FirstBend firstBend = firstBend();
                    GantryDataBipolarCo.SecondBend secondBend = secondBend();
                    ...
                    List<Point2> tps = CctUtils.trackingPhaseEllipse(
                            trajectory2.getLength(), xPlane,
                            delta, 16, false, 0.1,
                            elements, trajectory2, 215
                    );
        ...
        List<Point2> cps = CctUtils.cosyPhaseEllipse(
                xPlane, delta, 512, 5,
                false, 0.1, COSY_MAP1021.part2.map
        );
    ...
    @Run(-1000) // important
    public void 紧急事件磁场均匀性分析之傅里叶() {
        GantryDataBipolarCo.FirstBend firstBend = firstBend();
        GantryDataBipolarCo.SecondBend secondBend = secondBend();

        Cct di = dipoleCct345_local(secondBend);
        Cct ag = agCct345_local(secondBend);
        ...
        /**
         * Point3 center = ArcLine.unitCircle(BaseUtils.Converter.angleToRadian(67.5/2))
         *                 .toVector2().dot(0.95).toPoint2().toPoint3();
         *
         * or
         *
         * Point3 center = ArcLine.unitCircle(BaseUtils.Converter.angleToRadian(0.0))
         *                 .toVector2().dot(0.95).toPoint2().toPoint3();
         */
        Point3 center = ArcLine.unitCircle(BaseUtils.Converter.angleToRadian(0.0))
                .toVector2().dot(0.95).toPoint2().toPoint3();

        Vector3 dx = Vector2.from(Point2.origin()).to(center.toPoint2()).toVector3().normalSelf();

        Vector3 dy = Vector3.getZDirect().normalSelf();

        List<Point3> point3s = BaseUtils.Python.linspaceStream(0, 2 * Math.PI, 512)
                .mapToObj(t -> center.copy().moveSelf(
                        dx.dot(Math.cos(t)).add(dy.dot(Math.sin(t))).changeLengthSelf(60 * MM)
                ))
                .collect(Collectors.toList());

//        cct.plot3();
//
//        Plot3d.plot3(point3s, Plot2d.RED_LINE);
//
//        Plot3d.showThread();

        double[] zData = point3s.stream().map(cct::magnetAt).mapToDouble(Vector3::getZ).toArray();

        List<Point2> forward = Arrays.stream(FFT.forwardToAmplitude(zData)).limit(8).collect(Collectors.toList());

        double max = forward.stream().mapToDouble(Point2::getY).max().orElse(0);

        forward = Point2.convert(forward, MathFunction.identity(), y -> y / max);

        forward.forEach(p -> System.out.printf("%d极场分量%f%n", (int) (p.x * 2), p.y));

        Plot2d.plot2(forward, Plot2d.BLACK_POINT);

        Plot2d.showThread();
    }

    @Run(-1001) // important
    public void 紧急事件磁场均匀性分析之热图() {
        GantryDataBipolarCo.FirstBend firstBend = firstBend();
        GantryDataBipolarCo.SecondBend secondBend = secondBend();
        ...
        Point3 center = ArcLine.unitCircle(BaseUtils.Converter.angleToRadian(0))
                .toVector2().dot(0.95).toPoint2().toPoint3();
        ...
        String data = BaseUtils.Python.linspaceStream(-60 * MM, 60 * MM, 121)
                .mapToObj(y -> {
                    BaseUtils.StreamTools.Average avg = BaseUtils.StreamTools.Average.create();
                    return BaseUtils.Python.linspaceStream(-60 * MM, 60 * MM, 121)
                            .mapToObj(x -> {
                                Point3 p = center.copy().moveSelf(
                                        dx.dot(x).add(dy.dot(y))
                                );

                                return x * x + y * y > 60 * MM * 60 * MM ? Vector3.getZero() : ag.magnetAt(p);
                            })
                            .mapToDouble(Vector3::getZ)
//                            .peek(avg::add)
//                            .map(b -> BaseUtils.Equal.isEqual(b, 0) ? -1.195 : b)
    ...
    public void 二极CCT_bx磁场分布(){
        ...
        SoleLayerCct soleLayerCct1 = cct.get(1);

        List<Point2> bz1 = soleLayerCct.leftHandMagnetAlongTrajectory(trajectory);
        List<Point2> bz2 = soleLayerCct1.leftHandMagnetAlongTrajectory(trajectory);
        List<Point2> bz = cct.leftHandMagnetAlongTrajectory(trajectory);
    ...
    public void 四极CCT_bx磁场分布(){
        ...
        Cct soleLayerCct = cct.getCcts(0,3);
        Cct soleLayerCct1 = cct.getCcts(3,6);
    ...
    public Elements allElementPart2(GantryDataBipolarCo.FirstBend firstBend, GantryDataBipolarCo.SecondBend secondBend) {
        Cct all = dipoleCct345_local(secondBend).addCct(agCct345_local(secondBend));
    ...
    private Cct dipoleCct345_local(GantryDataBipolarCo.SecondBend secondBend) {
    ...
    private Cct agCct345_local(GantryDataBipolarCo.SecondBend secondBend) {
        Cct cct = Cct.getEmptyCct();
    ...
    private void plotPhiKsiFun(GantryDataBipolarCo.SecondBend secondBend, String desInner, String desOuter) {
        final double bigR = secondBend.agCct345BigR;
...
//        SoleLayerCct soleLayerCctInner1 = SoleLayerCct.createSoleLayerCct(bigR, innerSmallR, phiKsiFunInner1, I0,
//                0, endKsis[0], windingNumbers[0] * 360);
        Plot2d.plot2(phiKsiFunInner1, Point2.create(0, endKsis[0]), windingNumbers[0] * 360, desInner);

        MathFunction phiKsiFunInner2 = MathFunction.createPhiKsiFun(bigR, innerSmallR, phi0[1], BaseUtils.ArrayUtils.dot(tiltAngles, -1))
                .yAxisSymmetry().move(Vector2.create(endKsis[0], bendingRadians[0] + phi0[0]));
//        SoleLayerCct soleLayerCctInner2 = SoleLayerCct.createSoleLayerCct(bigR, innerSmallR, phiKsiFunInner2, I0,
//                endKsis[0], endKsis[0] - endKsis[1], windingNumbers[1] * 360);
        Plot2d.plot2(phiKsiFunInner2, Point2.create(endKsis[0], endKsis[0] - endKsis[1]), windingNumbers[1] * 360, desInner);
...
//        SoleLayerCct soleLayerCctInner3 = SoleLayerCct.createSoleLayerCct(bigR, innerSmallR, phiKsiFunInner3, I0,
//                endKsis[0] - endKsis[1], endKsis[0] - endKsis[1] + endKsis[2], windingNumbers[2] * 360);
        Plot2d.plot2(phiKsiFunInner3, Point2.create(endKsis[0] - endKsis[1], endKsis[0] - endKsis[1] + endKsis[2]), windingNumbers[2] * 360, desInner);
...
//        SoleLayerCct soleLayerCctOuter1 = SoleLayerCct.createSoleLayerCct(bigR, outerSmallR, phiKsiFunOuter1, I0,
//                Math.PI, Math.PI + endKsis[0], windingNumbers[0] * 360);
        Plot2d.plot2(phiKsiFunOuter1, Point2.create(Math.PI, Math.PI + endKsis[0]), windingNumbers[0] * 360, desOuter);
        ...
        SoleLayerCct soleLayerCctOuter2 = SoleLayerCct.createSoleLayerCct(bigR, outerSmallR, phiKsiFunOuter2, I0,
                endKsis[0] + Math.PI, endKsis[0] - endKsis[1] + Math.PI, windingNumbers[1] * 360);
        Plot2d.plot2(phiKsiFunOuter2, Point2.create(endKsis[0] + Math.PI, endKsis[0] - endKsis[1] + Math.PI), windingNumbers[1] * 360, desOuter);
...
//        SoleLayerCct soleLayerCctOuter3 = SoleLayerCct.createSoleLayerCct(bigR, outerSmallR, phiKsiFunOuter3, I0,
//                endKsis[0] - endKsis[1] + Math.PI, endKsis[0] - endKsis[1] + endKsis[2] + Math.PI, windingNumbers[2] * 360);
        Plot2d.plot2(phiKsiFunOuter3, Point2.create(endKsis[0] - endKsis[1] + Math.PI, endKsis[0] - endKsis[1] + endKsis[2] + Math.PI), windingNumbers[2] * 360, desOuter);
    ...
    private GantryDataBipolarCo.SecondBend secondBend() {
        ...
        secondBend.agCctWindingNumber3 = 24;
