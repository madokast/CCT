...
public class A0603分析真实磁场和光学设计差距 {
    @run(1)
    public void 适应性研究_先分析下单粒子() {
        ...
        CosyMap第二段.drawElement(20, 20, Plot2d.GREY_DASH, Plot2d.GREY_DASH);

        Plot2d.plotXLine(0, 0, trajectoryPart2.getLength(), Plot2d.GREY_DASH);
...
//        agCct12IOuter = 0;
        ...
        CosyMap第一段.drawElement(2, 2, Plot2d.GREY_DASH, Plot2d.GREY_DASH);

        Plot2d.plotXLine(0, 0, trajectoryPart1.getLength(), Plot2d.GREY_DASH);
...
//        dipoleCct345IOuter = 0;
        ...
        String[] strings = BaseUtils.Python.linspaceStream(0.005, 0.0075, switcher.getSize())
                .sequential()
                .mapToObj(k -> {
                    agCct345A0Inner = k;
                    agCct345A0Outer = -k;
                ...
                .map(bi -> {
                    ...
                    List<Point2> x = trackingIdealParticle(trajectoryPart2, trajectoryPart2.getLength(), magnetAble, true);
                ...
                .map(bi -> {
                    ...
                    Plot2d.plot2(mm2mmm(x), switcher.getAndSwitch());
    ...
    public void 四极CCT中加入二极场_磁场() {
        ...
        agCct345A0Inners = null;
        agCct345A0Outers = null;

        String[] le = BaseUtils.Python.linspaceStream(0.0, 1, 2)
                .sequential()
                .mapToObj(k -> {
                    agCct345A0Inners = new double[]{
                            k * agCct345SmallRInner,
                            -k * agCct345SmallRInner,
                            k * agCct345SmallRInner
                    };
                ...
                .map(bi -> {
                    ...
                    return BaseUtils.Content.BiContent.create(k, bz);
                })
                .collect(Collectors.toList())
                .stream()
                .sorted(Comparator.comparingDouble(BaseUtils.Content.BiContent::getT1))
                .sequential()
                .peek(bi -> {
                    List<Point2> bz = bi.getT2();
                    Plot2d.plot2(bz, switcher.getAndSwitch());
                })
                .map(BaseUtils.Content.BiContent::getT1)
                .map(k -> String.format("k=%5.2f", k))
                .collect(Collectors.toList())
                .toArray(String[]::new);
    ...
    public void 完全消除四极CCT的二极场() {
        ...
        String[] le = BaseUtils.Python.linspaceStream(-0.1, 0.1, switcher.getSize())
                .sequential()
                .mapToObj(k -> {
                    agCct345A0Inners = new double[]{
...
//        dipoleCct345IOuter = 0;
        ...
        Plot2d.info("s/m", "x/mm", "参考粒子跟踪-引入四极CCT二极场", 18);
        Plot2d.legend(18, le);
    ...
    public void 二极CCT二极场分布() {
        ...
        Plot2d.plot2(bz, Plot2d.BLUE_LINE);

        Plot2d.info("s/m", "B0/T", "二极CCT二极场分布", 18);

        CosyMap第二段.drawElement(3.5, 3.5);
    ...
    public void 直接移动四极CCT看看() {
        ...
        String[] le = BaseUtils.Python.linspaceStream(0 * MM, 10 * MM, switcher.getSize())
                ...
                .mapToObj(s -> {
                    ...
                    return BaseUtils.Content.BiContent.create(s, elements);
                })
                .collect(Collectors.toList())
                .stream()
                .parallel()
...
//                    List<Point2> x = elements.magnetBzAlongTrajectory(trajectoryPart2, 10 * MM);

                    return BaseUtils.Content.BiContent.create(s, mm2mmm(x));
                })
                .collect(Collectors.toList())
                .stream()
                .sequential()
                ...
                .map(BaseUtils.Content.BiContent::getT1)
                .map(s -> String.format("QUAD-CCT OFFSET %3.2fMM", s / MM))
                .collect(Collectors.toList())
                .toArray(String[]::new);
    ...
    public void 移动四极CCT875MM() {
        ...
        if (false) {
            List<Point2> x = trackingIdealParticle(trajectoryPart2, trajectoryPart2.getLength(), elements, true);
            List<Point2> xO = trackingIdealParticle(trajectoryPart2, trajectoryPart2.getLength(), elementsOrigin, true);
            List<Point2> xN = trackingIdealParticle(trajectoryPart2, trajectoryPart2.getLength(), elementsNoQuad, true);

            Plot2d.plot2(mm2mmm(x), Plot2d.BLACK_LINE);
            Plot2d.plot2(mm2mmm(xO), Plot2d.RED_LINE);
            Plot2d.plot2(mm2mmm(xN), Plot2d.BLUE_LINE);


            Plot2d.info("s/m", "x/mm", "移动四极CCT", 18);
            Plot2d.legend(18, "QUAD-CCT OFFSET 8.75MM", "ORIGIN", "ORIGIN WITHOUT QUAD-CCT");
        }
    ...
    public void 在入口添加磁场() {
        ...
        String[] le = BaseUtils.Python.linspaceStream(0.5, 1.5, switcher.getSize())
                ...
                .mapToObj(k -> {
                    ...
                    return BaseUtils.Content.BiContent.create(k, help);
                })
                .collect(Collectors.toList())
                .stream()
                .parallel()
                .map(bi -> {
                    ...
                    return BaseUtils.Content.BiContent.create(k, bzh);
                })
                .collect(Collectors.toList())
                .stream()
                .sorted(Comparator.comparingDouble(BaseUtils.Content.BiContent::getT1))
                .peek(bi -> Plot2d.plot2(bi.getT2()))
                .map(BaseUtils.Content.BiContent::getT1)
                .map(k -> String.format("ANG=%5.3f", k))
                .collect(Collectors.toList())
                .toArray(String[]::new);


        Plot2d.info("s/m", "B0/T", "辅助线圈", 18);

        Plot2d.legend(18, BaseUtils.ArrayUtils.append(new String[]{"DI-CCT"}, le));

        Plot2d.plotXLine(0, 0, 3, Plot2d.GREY_DASH);

        Plot2d.showThread();
    }

    @run(11)
    public void 在入口添加磁场粒子跟踪() {
        Trajectory testTrajectory = getTestTrajectory(0.75, 67.5);
...
//        Plot2d.plot2(bz, Plot2d.BLACK_SMALL_POINT);
        ...
        List<Point2> x = trackingIdealParticle(testTrajectory, testTrajectory.getLength(), dipoleCct345, true);
        Plot2d.plot2(Point2.convert(mm2mmm(x), 1, -1), Plot2d.BLACK_SMALL_POINT);
...
//                    List<Point2> bzh = help.magnetBzAlongTrajectory(testTrajectory);
                    ...
                    List<Point2> xh = trackingIdealParticle(testTrajectory, testTrajectory.getLength(), empty, true);

                    return BaseUtils.Content.BiContent.create(k, Point2.convert(mm2mmm(xh), 1, -1));
                })
                .collect(Collectors.toList())
                .stream()
                .sorted(Comparator.comparingDouble(BaseUtils.Content.BiContent::getT1))
...
//                testTrajectory.directAtEnd().copy().reverseSelf().toVector3(),
//                250.0
//        );
//        List<Point3> run = ParticleRunner.run(ip, dipoleCct345, testTrajectory.getLength(), MM);


//        testTrajectory.plot3d();
//        dipoleCct345.plot3();
    ...
    public void 辅助线圈实战() {
        ...
        String[] le = BaseUtils.Python.linspaceStream(0.05, 0.15, switcher.getSize())
                ...
                .mapToObj(k -> {
                    ...
                    return BaseUtils.Content.BiContent.create(k, empty);
                })
                .collect(Collectors.toList())
                .stream()
                .parallel()
                .map(bi -> {
                    ...
                    return BaseUtils.Content.BiContent.create(k, mm2mmm(x));
                })
                .collect(Collectors.toList())
                .stream()
                .sequential()
...
//        agCct345IInner = 0;
    ...
    private void phase相椭圆画图(
        ...
        for (int i = 0; i < lists.size(); i++) {
            List<Point2> list1 = lists.get(i);
            Plot2d.plot2(list1, describes.get(i));
        }
    ...
    private Trajectory getTestTrajectory(double BigR, double angle) {
        return TrajectoryFactory.setStartingPoint(BigR, -1.0)
