 ...
 * Description
 * 前情提要：
 * 我们用遗传算法匹配CCT，只匹配 DL2 + CCT345 + GAP3 的一段
 * 匹配目标是 COSY 光学设计在 QS3 前的结果
 * <p>
 * 在这次匹配中，有以下特点：
 * 1. 不再使用 3.5 * 7.5 作为后偏转段的输入，而实以 COSY 在第二段入口的粒子数据作为 track 的输入
 * 2. 只匹配 CCT，即 DL2 + CCT345 + GAP3 的一段
 * 3. 匹配目标不再是束斑位置和束斑大小，而是对比 track 和 COSY 的每一个粒子，目标为 track 粒子和 cosy 粒子的相空间坐标偏差
 * 即 |x_track - x_cosy| |xp_track - xp_cosy| |y_track - y_cosy| |yp_track - yp_cosy|
 * 4. 以上的粒子，当前取值是 x 平面和 y 平面，动量分散 0 和 5%，相空间上均匀取 2-5 个粒子（实际粒子数目是 (2-5)*2*2 = (8-20) ）
 * <p>
 * 匹配结果：有比较符合解
 * <p>
 * 当前目标：既然 CCT 匹配好了，那就匹配 QS，QS 只有两个参数，所以手调？
 * 先不说手调，先看看情况如何
 *
 * <p>
 * Data
...
public class C20201118CUDA匹配结果 {
    ...
    static Trajectory t = GantryDataBipolarCoUtils.getTrajectory2(
            GantryDataBipolarCo.FirstBend.getDefault(), GantryDataBipolarCo.SecondBend.getDefault());

    static GantryDataBipolarCo.SecondBend defaultSecond = GantryDataBipolarCo.SecondBend.getDefault();

    double LEN_TO_QS3 = defaultSecond.DL2 + defaultSecond.CCT345_LENGTH + defaultSecond.GAP3;
    ...
    public void 直接看ISOC() {
        ...
        List<PhaseSpaceParticle> pps_cosy_qs3_dp1 = pp_cosy_after(COSY_MAP1021.all.map, 512, xPlane, dp_1);
        List<PhaseSpaceParticle> pps_cosy_qs3_dp2 = pp_cosy_after(COSY_MAP1021.all.map, 512, xPlane, dp_2);
        List<PhaseSpaceParticle> pps_cosy_qs3_dp3 = pp_cosy_after(COSY_MAP1021.all.map, 512, xPlane, dp_3);
        ...
        ParticleRunner.runThread(ps_dp2, e, LEN_ALL_PART2, MM);
        ParticleRunner.runThread(ps_dp3, e, LEN_ALL_PART2, MM);
    ...
    private List<PhaseSpaceParticle> pp_cosy_after(CosyArbitraryOrder.CosyMapArbitraryOrder map,
                                                   int number, boolean xPlane, double dp) {
        List<PhaseSpaceParticle> pp = PhaseSpaceParticles.phaseSpaceParticlesAlongPositiveEllipseInPlane(
                xPlane, 3.5 * MM, 7.5 * MM, dp, number);

        // 转为COSY能量分散
        pp = PhaseSpaceParticles.convertDeltaFromMomentumDispersionToEnergyDispersion(pp, 215);

        // 运行
        List<PhaseSpaceParticle> ppEnd = map.apply(pp, 5);
    ...
    private static volatile List<PhaseSpaceParticle> COSY_PARTICLES_ISOC = null;

    public static synchronized List<PhaseSpaceParticle> cosyParticleISOC() {

        if (COSY_PARTICLES_ISOC == null) {
            int number = PARTICLE_NUMBER_PER_PLANE_PER_DP;
            ...
            // 运行 TODO all beforeQS3WithPart1
            List<PhaseSpaceParticle> ppEnd = COSY_MAP1021.all.map.apply(pp, 5);

            // 能量分散转回去
            COSY_PARTICLES_ISOC = PhaseSpaceParticles.convertDeltaFromEnergyDispersionToEnergyDispersionMomentumDispersion(
                    ppEnd, 215);
        ...
        return PhaseSpaceParticles.copy(COSY_PARTICLES_ISOC);
    }
