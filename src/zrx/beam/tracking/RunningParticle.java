...
//                this.plot3self(describe);
    ...
    /**
     * 打印粒子在相空间的坐标 x x' y y'
     * @param cs 粒子自然坐标系，用CCT坐标系为基表示
     * @param idealP 理想例子
     */
    public void printPhaseSpacecoordinate(CoordinateSystem3d cs,final RunningParticle idealP){
        final Vector2d xxc = this.phaseSpaceParticle(cs, idealP).getXXCAsVector2d();
        final Vector2d yyc = this.phaseSpaceParticle(cs, idealP).getYYCAsVector2d();

        System.out.println("x = "+ xxc.x);
        System.out.println("x' = "+ xxc.y);
        System.out.println("y = "+ yyc.x);
        System.out.println("y' = "+ yyc.y);
    }
