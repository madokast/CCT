...
//        double dphi = cn * Math.sin(this.nth * dksi) + this.phi0 * dksi / (2.0 * Math.PI);
     ...
     * @param cct 原来的 CCT
     */
    public static void reverseWinding(CurvedCCT cct) {
        double endPhi = cct.getEndPhi();
        double endKsi = cct.getEndTheta();
        double step = -cct.stepKsi;
        double tiltAngle = -cct.tiltAngle;
