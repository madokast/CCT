...
public class 六十七点五新测试0804 {
    ...
    //匝数
    private static final int N12_OUT = 32;//  8.973/67.5*241 = 31.904
    private static final int N12_MID = 46;//  12.8778/67.5*241 = 45.9785155555556
    private static final int N12_IN = 85;//  23.7984/67.5*241 = 84.9691022222222
    ...
    //电流
    private static final double I1 = 6516.0;
    private static final double I2 = -I1;
    private static final double I3 = -5134.4;
...
//        Plot2d.equal();
    ...
    private static void ellipseTracking(AllCCTs allCCTs) {
        ...
        final int number = 10;
        //粒子局部坐标系
        CoordinateSystem3d cs = new CoordinateSystem3d(Vector3d.getXDirect(-1),
                Vector3d.getZDirect(),
                Vector3d.getYDirect());
...
//        Plot2d.plot2(points,Plot2d.YELLOW_POINT);
        ...
        final RunningParticle[] particles = ParticleFactory.copy(particle, number);

        switch (flag){
            case 1:
                for (int i = 0; i < number; i++) {
                    particles[i].deploy(cs,ellipsePoints[i].x,ellipsePoints[i].y,0,0,0);
    ...
    private static void particleTracking(AllCCTs allCCTs) {
        //粒子生成
        RunningParticle particle = ParticleFactory.getProton(
                Vector3d.getOne(1, -1, 0),//position
                Vector3d.getOne(0, 1, 0),//direction
